...
class ProductRepo extends DataSyncRepo {
  ...
  Future<ApiResponseModel> getBrandOrCategoryProductList(
    ...
    try {
      final response = await dioClient.get(
        '${AppConstants.categoryProductUri}$id?limit=10&&offset=$offset',
      );
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponseModel<T>> getRelatedProducts<T>(
    String productId, {
    required int offset,
    required DataSourceEnum source,
  }) async {
    return await fetchData<T>(
      '${AppConstants.relatedProductsUri}$productId?limit=10&offset=$offset',
      source,
    );
  }
}
