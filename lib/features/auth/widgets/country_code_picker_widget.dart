...
// List of Gulf countries + Yemen
const List<String> allowedCountryCodes = [
  'YE', // Yemen
  'SA', // Saudi Arabia
  'AE', // UAE
  'KW', // Kuwait
...
class CountryCodePickerWidget extends StatefulWidget {
  ...
  const CountryCodePickerWidget({
    this.onChanged,
    this.onInit,
    this.initialSelection = 'YE', // Default to Yemen
    this.favorite = const ['SA', 'AE', 'KW', 'QA', 'BH', 'OM'],
    this.textStyle,
    ...
    this.comparator,
    this.countryFilter =
        allowedCountryCodes, // Filter to allowed countries only
  ...
  State<StatefulWidget> createState() {
    // Filter countries to only Gulf countries + Yemen
    List<Map<String, String>> jsonList = countryList.where((country) {
      final countryCode = country['code'];
      return allowedCountryCodes.contains(countryCode?.toUpperCase());
    }).toList();
...
class CountryCodePickerWidgetState extends State<CountryCodePickerWidget> {
        ...
        selectedItem = elements.firstWhere(
          (criteria) =>
              ...
              (criteria.name!.toUpperCase() ==
                  widget.initialSelection!.toUpperCase()),
          orElse: () => elements.firstWhere(
            (element) => element.code == 'YE', // Default to Yemen if not found
            orElse: () => elements[0],
          ),
        );
      } else {
        selectedItem = elements.firstWhere(
          (element) => element.code == 'YE', // Default to Yemen
          orElse: () => elements[0],
        );
      }
      _onInit(selectedItem);
  ...
  void initState() {
    ...
    // Set default selection to Yemen (YE)
    if (widget.initialSelection != null) {
      selectedItem = elements.firstWhere(
        (item) =>
    ...
    // Filter favorite elements to only allowed countries
